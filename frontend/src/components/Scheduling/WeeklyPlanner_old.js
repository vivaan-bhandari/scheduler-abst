...
const WeeklyPlanner = forwardRef(({ facilityId, refreshTrigger }, ref) => {
  ...
  const [currentWeek, setCurrentWeek] = useState(() => {
    // Always use the global selected week from WeekContext (July 21, 2025 for Murray Highland)
    const globalWeek = selectedWeek ? new Date(selectedWeek) : new Date('2025-07-21');
    console.log('🔍 DEBUG WeeklyPlanner: Using global selectedWeek:', selectedWeek);
    console.log('🔍 DEBUG WeeklyPlanner: Converted to Date object:', globalWeek.toISOString().split('T')[0]);
    return globalWeek;
  });
  ...
  // Sync with global week selection - ALWAYS use the global week
  useEffect(() => {
    const newWeek = selectedWeek ? new Date(selectedWeek) : new Date('2025-07-21');
    setCurrentWeek(newWeek);
    console.log('🔍 DEBUG WeeklyPlanner: Synced with global week:', selectedWeek);
    console.log('🔍 DEBUG WeeklyPlanner: New currentWeek set to:', newWeek.toISOString().split('T')[0]);
  }, [selectedWeek]);

  // Watch for refresh trigger changes
  useEffect(() => {
    if (refreshTrigger > 0) {
      console.log('🔍 WeeklyPlanner: refreshTrigger changed to:', refreshTrigger);
      console.log('🔍 WeeklyPlanner: Triggering refresh due to refreshTrigger change');
      fetchData();
    }
  }, [refreshTrigger]);
  ...
  const fetchData = async () => {
    console.log('🔍 DEBUG WeeklyPlanner: fetchData called');
    console.log('🔍 DEBUG WeeklyPlanner: facilityId:', facilityId);
    console.log('🔍 DEBUG WeeklyPlanner: currentWeek:', currentWeek?.toISOString());
    console.log('🔍 DEBUG WeeklyPlanner: week start:', getWeekStart(currentWeek));
    
    try {
      console.log('🔍 DEBUG WeeklyPlanner: Making API calls...');
      const weekStart = getWeekStart(currentWeek);
      console.log('🔍 DEBUG WeeklyPlanner: Week start for API call:', weekStart);
      
      const [shiftsRes, staffRes, assignmentsRes] = await Promise.all([
        axios.get(`${API_BASE_URL}/api/scheduling/shifts/weekly/?facility=${facilityId}&week_start=${weekStart}`),
      ...
      console.log('Shifts details:', shiftsData.map(shift => ({
        id: shift.id,
        date: shift.date,
        shift_type: shift.shift_template?.shift_type,
  ...
  // Expose refreshData method and current week to parent component
  useImperativeHandle(ref, () => ({
    refreshData: () => {
      console.log('🔍 WeeklyPlanner: refreshData called from parent');
      console.log('🔍 WeeklyPlanner: Current state before refresh:', {
        facilityId,
  ...
  const getWeekStart = (date) => {
    // Force week start to July 21, 2025 to match AI recommendations
    const weekStart = '2025-07-21';
    
    console.log('🔍 DEBUG getWeekStart: Using fixed week start:', weekStart);
    
    return weekStart;
  };

  const getWeekDays = () => {
    // Force the week to start on July 21, 2025 (Monday) to match AI recommendations
    const weekStart = '2025-07-21';
    const days = [];
    for (let i = 0; i < 7; i++) {
    ...
    console.log('🔍 getWeekDays: Using fixed week start:', weekStart);
    console.log('🔍 getWeekDays: Generated days:', days.map(d => d.toISOString().split('T')[0]));
  ...
  const getShiftForDay = (day, shiftType) => {
    ...
    const foundShift = shifts.find(shift => 
      shift.date === dayStr && shift.shift_template?.shift_type === shiftType
    );
    
    console.log(`🔍 getShiftForDay(${dayStr}, ${shiftType}):`, {
      dayStr,
      shiftType,
      totalShifts: shifts.length,
      foundShift: foundShift ? {
        id: foundShift.id,
        date: foundShift.date,
        shift_type: foundShift.shift_template?.shift_type,
        required_staff_count: foundShift.required_staff_count
      } : null
    });
  ...
  const handleApplyAIRecommendations = async () => {
    ...
    try {
      ...
      console.log('🔍 WeeklyPlanner: Applying AI recommendations for week start:', weekStart);
      console.log('🔍 WeeklyPlanner: Current week:', currentWeek.toISOString().split('T')[0]);
      console.log('🔍 WeeklyPlanner: Facility ID:', facilityId);
      ...
      // Refresh the data to show the applied recommendations
      fetchData();
    ...
    <Box sx={{ 
      ...
      <Box sx={{ 
        ...
        <Box sx={{ 
          ...
          <Button
            ...
            disabled={loading || !hasAIRecommendations}
      ...
      <Box sx={{ 
        ...
        <Box sx={{ 
          ...
          <Button
            variant="contained"
            startIcon={applying ? <AIIcon /> : <AIIcon />}
            onClick={handleApplyAIRecommendations}
            disabled={applying || !facilityId}
            sx={{ 
              fontWeight: '600',
              bgcolor: '#4caf50',
          ...
          >
            {applying ? 'Applying AI...' : 'Apply AI Recommendations'}
          </Button>
      ...
      {/* Staff Section - Clean Minimal Design */}
      <Paper sx={{ 
        p: 4, 
        mb: 3, 
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
          <Box>
            ...
            <Typography variant="body1" color="text.secondary" sx={{ fontSize: '0.95rem' }}>
              {filteredStaff.length} available staff members
            </Typography>
          </Box>
          ...
          <Box sx={{ display: 'flex', gap: 4 }}>
            <Box sx={{ textAlign: 'center' }}>
              <Typography variant="h3" sx={{ fontWeight: '700', color: '#10b981', mb: 0.5 }}>
                {filteredStaff.filter(m => getStaffStatus(m) === 'available').length}
              </Typography>
        ...
        <Box sx={{ mb: 4 }}>
          <TextField
            ...
            InputProps={{
              startAdornment: <SearchIcon sx={{ mr: 2, color: '#6b7280' }} />
            }}
          />
          
          <FormControl sx={{ ml: 3, minWidth: 140 }}>
            <Select
              value={roleFilter}
        ...
        <Box sx={{ 
          ...
          {filteredStaff.map((member) => {
            ...
            const hoursRemaining = member.max_hours - hoursUsed;
              ...
              >
                ...
                <Box sx={{ mb: 3 }}>
                  ...
                  <Box sx={{ 
                    ...
                    <Box sx={{
                      width: `${Math.min(100, (hoursUsed / member.max_hours) * 100)}%`,
                      height: '100%',
                      bgcolor: '#3b82f6',
                      ...
                      {staffAssignments.slice(0, 2).map((assignment) => {
                        ...
                        if (assignedShift) {
                          ...
                          return (
                            ...
                            >
                              {day} {assignedShift.shift_type?.toUpperCase()}
                            </Box>
                          );
                        }
                        return null;
                            ...
                            {shift && shift.id ? (
                              <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', position: 'relative' }}>
                                ...
                                <Box sx={{ 
                                  ...
                                  {/* MedTech requirement */}
                                  <Chip 
                                    label={`MedTech: ${shiftAssignments.filter(a => {
                                      const staffMember = staff.find(s => s.id === a.staff);
                                      return staffMember?.role === 'med_tech';
                                    }).length}/1`}
                                    size="small"
                                    variant="outlined"
                                    sx={{ 
                                  ...
                                  <Chip 
                                    label={`Caregiver: ${shiftAssignments.filter(a => {
                                      const staffMember = staff.find(s => s.id === a.staff);
                                      return staffMember?.role === 'caregiver';
                                    }).length}/${Math.max(0, (shift.required_staff_count || 1) - 1)}`}
                                    size="small"
                                    variant="outlined"
                                    sx={{ 
                                      fontWeight: '500',
                                ...
                                {/* Role-Specific Staff Needed Text */}
                                <Typography variant="caption" display="block" sx={{ mb: 1, fontWeight: 'medium' }}>
                                  {(() => {
                                    const medTechAssigned = shiftAssignments.filter(a => {
                                      const staffMember = staff.find(s => s.id === a.staff);
                                      return staffMember?.role === 'med_tech';
                                    }).length;
                                    
                                    const caregiverAssigned = shiftAssignments.filter(a => {
                                      const staffMember = staff.find(s => s.id === a.staff);
                                      return staffMember?.role === 'caregiver';
                                    }).length;
                                    
                                    const medTechNeeded = Math.max(0, 1 - medTechAssigned);
                                    const caregiverNeeded = Math.max(0, ((shift.required_staff_count || 1) - 1) - caregiverAssigned);
                                    
                                    if (medTechNeeded === 0 && caregiverNeeded === 0) {
                                      return 'Filled';
                                    }
                                    
                                    const needs = [];
                                    if (medTechNeeded > 0) needs.push(`${medTechNeeded} MedTech${medTechNeeded > 1 ? 's' : ''}`);
                                    if (caregiverNeeded > 0) needs.push(`${caregiverNeeded} Caregiver${caregiverNeeded > 1 ? 's' : ''}`);
                                    
                                    return `${needs.join(', ')} needed`;
                                  })()}
                                </Typography>
